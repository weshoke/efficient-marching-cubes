...
: parseErrorType(NO_SYNTAX_ERROR), evalErrorType(0), data(new Data)
{
    data->referenceCounter = 1;
    using_var = (bool *)NULL;
}
...
: parseErrorType(cpy.parseErrorType), evalErrorType(cpy.evalErrorType),
  data(cpy.data)
{
...
: useDegreeConversion(false), ByteCode(0), ByteCodeSize(0), Immed(0),
  ImmedSize(0), Stack(0), StackSize(0)
{
}
...
: varAmount(cpy.varAmount), useDegreeConversion(cpy.useDegreeConversion),
  ...
  ByteCode(0), ByteCodeSize(cpy.ByteCodeSize), Immed(0),
  ImmedSize(cpy.ImmedSize), Stack(0), StackSize(cpy.StackSize)
{
...
// Function pointers
bool FunctionParser::AddFunction(const std::string &name, FunctionPtr func,
...
FunctionParser::FindVariable(const char *F, const Data::VarMap_t &vars) const
{
    if (vars.size()) {
        ...
        if (ind) {
            string name(F, ind);
            FunctionParser::Data::VarMap_t::const_iterator vIter =
                vars.find(name);
            if (vIter != vars.end()) using_var[vIter->second - VarBegin] = true;
            return vIter;
        }
    }
    return vars.end();
...
int FunctionParser::CheckSyntax(const char *Function)
{
    ...
    while (true) {
        ...
        // Check for operator
        if (!IsOperator(c) && (c != ',' || functionParenthDepth.empty() ||
                               functionParenthDepth.back() != ParenthCnt)) {
            parseErrorType = EXPECT_OPERATOR;
            return Ind;
        }

        // If we get here, we have an operand and an operator; the next loop
        // will
...
bool FunctionParser::Compile(const char *Function)
{
    ...
    if (data->ImmedSize) {
        ...
        memcpy(data->Immed, &immed[0], sizeof(float) * data->ImmedSize);
    }
    if (data->StackSize) data->Stack = new float[data->StackSize];
...
inline void FunctionParser::AddImmediate(float i) { tempImmed->push_back(i); }
...
int FunctionParser::CompileElement(const char *F, int ind)
{
    ...
    if (isalpha(c) || c == '_')  // Function, variable or constant
    {
...
#ifndef DISABLE_EVAL
            unsigned requiredParams = strcmp(func->name, "eval") == 0
                                          ? data->Variables.size()
                                          : func->params;
#else
        ...
        if (pIter !=
            data->FuncParserNames.end())  // is user-defined func parser
        {
...
//===========================================================================
namespace {
inline int floatToInt(float d) { return d < 0 ? -int((-d) + .5) : int(d + .5); }
inline float Min(float d1, float d2) { return d1 < d2 ? d1 : d2; }
inline float Max(float d1, float d2) { return d1 > d2 ? d1 : d2; }
...
float FunctionParser::Eval(const float *Vars)
{
    ...
    for (IP = 0; IP < ByteCodeSize; ++IP) {
        switch (ByteCode[IP]) {
            ...
            case cAnd:
                Stack[SP - 1] =
                    (floatToInt(Stack[SP - 1]) && floatToInt(Stack[SP]));
                --SP;
                break;
            case cOr:
                Stack[SP - 1] =
...
struct CodeTreeData {
   ...
   public:
    ...
    inline float GetImmed() const { return value; }
...
#define CHECKCONSTNEG(item, op) \
    ((op) == cMul) ? (item).CheckConstInv() : (item).CheckConstNeg()
...
struct CodeTree {
   ...
   private:
    ...
    void OptimizeRedundant()
    {
        ...
        if (GetArgCount() == 1) {
            if (GetOp() == cMul || GetOp() == cAdd || GetOp() == cMin ||
                GetOp() == cMax)
...
#define ConstantBinaryFun(token, fun)                              \
    case token: {                                                  \
        ...
        break;                                                     \
                ...
                ConstantBinaryFun(cMod,
                                  fmod);  // not a func, but belongs here too
    ...
    void OptimizeAddMulFlat()
    {
        ...
        if (GetOp() == cAdd || GetOp() == cMul) {
            ...
            for (pit b, a = GetBegin(); a != GetEnd(); a = b) {
                ...
                // Note: OptimizeConstantMath1() would be a good thing to call
                // next.
            }
        }
    }
        ...
        */
        ...
        if (GetOp() == cPow) {
            ...
            if (p0->IsImmed() && p0->GetImmed() == CONSTANT_E &&
                ...
                if (p0->IsImmed() && p0->GetImmed() == 10.0 &&
                    cl.value == CONSTANT_L10I && foundposlog) {
   ...
   public:
    ...
    void Assemble(vector<unsigned> &byteCode, vector<float> &immed) const;
...
void CodeTree::Assemble(vector<unsigned> &byteCode, vector<float> &immed) const
{
    ...
    switch (GetOp()) {
        ...
        case cMul: {
            ...
            for (pcit a = GetBegin(); a != GetEnd(); ++a) {
                ...
                bool done = false;
                if (pa->IsImmed()) {
                    if (GetOp() == cMul && pa->data->IsInverted() &&
                        (pnega || opcount == 2)) {
                        CodeTree tmp = *pa;
...
SubTreeDecl((const CodeTree &b), (b)) SubTreeDecl((float value), (value))
