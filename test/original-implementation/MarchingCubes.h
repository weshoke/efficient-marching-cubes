...
#ifndef OLD_MARCHINGCUBES_H_
#define OLD_MARCHINGCUBES_H_
...
class MarchingCubes
   ...
   public:
     ...
     * \param data is the pointer to the external data, allocated as a
     * size_x*size_y*size_z vector running in x first
     */
   ...
   protected:
    /** tesselates one cube */
    void process_cube();
    /** tests if the components of the tesselation of the cube should be
   ...
   public:
    ...
    /**
     * interpolates the horizontal gradient of the implicit function at the
     * lower vertex of the specified cube
     * \param i abscisse of the cube
     * \param j ordinate of the cube
     * \param k height of the cube
     */
    ...
    /**
     * interpolates the longitudinal gradient of the implicit function at the
     * lower vertex of the specified cube
     * \param i abscisse of the cube
     * \param j ordinate of the cube
     * \param k height of the cube
    ...
    /**
     * interpolates the vertical gradient of the implicit function at the lower
     * vertex of the specified cube
     * \param i abscisse of the cube
     * \param j ordinate of the cube
     * \param k height of the cube
     */
    ...
    const int *x_verts() const { return _x_verts; }
    const int *y_verts() const { return _y_verts; }
    const int *z_verts() const { return _z_verts; }
    //-----------------------------------------------------------------------------
    // Elements
   protected:
    bool _originalMC; /**< selects wether the algorithm will use the enhanced
                         ...
                         MarchingCubes */
    bool _ext_data;   /**< selects wether to allocate data or use data from
                         another class */
    ...
    int *_x_verts; /**< pre-computed vertex indices on the lower horizontal
                      edge of each cube */
    int *_y_verts; /**< pre-computed vertex indices on the lower longitudinal
                      edge of each cube */
    int *_z_verts; /**< pre-computed vertex indices on the lower vertical
                      edge of each cube */
